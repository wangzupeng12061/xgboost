    ...
    try:
        ...
        with LoggerContext(logger, "初始化回测组件"):
            ...
            selector = StockSelector(
                model=model,
                n_stocks=config['backtest'].get('top_n', 50),
                method=config['backtest'].get('selection_method', 'top_n')
            )
            
            # 组合管理器
            portfolio = PortfolioManager(
                initial_capital=config['backtest'].get('initial_capital', 1000000),
                commission_rate=config['backtest'].get('commission_rate', 0.0003),
        ...
        with LoggerContext(logger, "绩效评估"):
            ...
            if len(daily_values) > 0:
                ...
                if portfolio_values.index.duplicated().any():
                    logger.info(f"  发现 {portfolio_values.index.duplicated().sum()} 个重复日期，正在去重...")
                    ...
                    try:
                        ...
                        if not benchmark_data.empty:
                            ...
                            if benchmark_data.index.duplicated().any():
                                logger.info(f"  基准数据发现 {benchmark_data.index.duplicated().sum()} 个重复日期，正在去重...")
